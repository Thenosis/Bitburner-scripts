/**Info*********************************
 * Title--------------------------------
 * contract-solver.js -v1.0
 * Bitburner -v3.0.1
 * 
 * Description--------------------------
 * I cannot remember who originally wrote this. If you see this code and recognize it,
 * please submit an urgent push request with the relevent information and I will merge it post haste.
 * 
...
export function getContracts(ns) {
  ...
  for (const host of getAllHosts(ns, "home", servers)) {
...
solvers["Encryption I: Caesar Cipher"] = (data) => {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (chars.includes(char)) {
      const start = chars.indexOf(char);
      const end = (start - shift + chars.length) % chars.length;
      answer += chars[end];
    } else {
      answer += char;
    }
...
solvers["Encryption II: Vigenère Cipher"] = (data) => {
  ...
  while (fullKeyword.length < str.length) {
    fullKeyword += keyword;
  }
  ...
  for (let i = 0; i < str.length; i++) {
    const char = str[i];
    if (chars.includes(char)) {
      const shift = chars.indexOf(fullKeyword[i]);
      const start = chars.indexOf(char);
      const end = (start + shift) % chars.length;
      answer += chars[end];
    } else {
      answer += char;
    }
