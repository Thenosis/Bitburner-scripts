 ...
 * 
 * Description--------------------------
 * Creates a map of servers in the game.
 * 
 ...
 * Updated code to work for v3.0.0.
 * Changed line 138. The old code was not compatible with the current version as the method call was depricated.
 * Updated to ns.format.number() method.
 * Left original comments denoting original writer as well as first contributor.
 * 
 * 
 * Contributor--------------------------
 * Thenosis - 07/25/2026
...
function PrintServerInfo(serverName, indent, prefix) {
	...
	if (controlMaxMoneyIndicator) {
		maxMoneyIndicator = " [Max Money" + ` $${_ns.format.number(serverinfo.moneyMax)}` + "] ";
	}
